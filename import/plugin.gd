...
## plugin.gd
##
## EditorImportPlugin for .baproto schema files.
## Integrates with Godot's import system to automatically generate GDScript
## classes from Build-A-Proto schema definitions.
##
...
func _import(
	...
	var binary_path := Binary.resolve()
	if binary_path.is_empty():
		push_error("[baproto] Binary not found. Cannot import %s" % source_file)
		return ERR_FILE_NOT_FOUND

	# Step 2: Get output directory from ProjectSettings
	var output_dir := ProjectSettings.get_setting(
	...
	if dir == null:
		push_error("[baproto] Cannot access parent directory of output: %s" % output_dir_fs)
		return ERR_FILE_CANT_OPEN

	if not DirAccess.dir_exists_absolute(output_dir_fs):
		var err := DirAccess.make_dir_recursive_absolute(output_dir_fs)
		if err != OK:
			push_error("[baproto] Failed to create output directory: %s (error %d)" % [output_dir_fs, err])
			return err
	...
	# Write a minimal GDScript stub
	save_file.store_string(
		"# Generated by baproto import plugin\n" +
		"# Source: %s\n" % source_file +
		"# Generated files: %d\n" % gen_files.size()
	)
	save_file.close()
...
## `_collect_generated_files` recursively scans the output directory for all
## generated .gd files and returns their absolute filesystem paths.
func _collect_generated_files(dir_path: String) -> PackedStringArray:
	var result := PackedStringArray()
	var dir := DirAccess.open(dir_path)

	if dir == null:
		push_warning("[baproto] Cannot open output directory for collection: %s" % dir_path)
		return result
