...
## runtime_test.gd
##
## Comprehensive test suite for `BitStream` runtime utilities. Tests bit-level
## serialization, varints, ZigZag encoding, and composite types.
##
...
func test_zigzag_encode_positive(params = use_parameters([
	...
	# When: ZigZag encoding is applied.
	var encoded := Bitstream.zigzag_encode(input)
